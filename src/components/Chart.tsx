...
export const Chart = () => {
  ...
  useEffect(() => {
    ...
    const sequence = async () => {
      while (isMounted) {
        ...
        if (!isMounted) break
        await opacityControls.start({ opacity: 0, transition: { duration: 0.3 } })
  ...
  return (
    <div
      className={
        'dark:shadow-none overflow-hidden dark:border-neutral-800 border-neutral-300 h-full w-full flex items-center justify-center'
      }
      // onMouseEnter removed for continuous loop
    >
      <div className={'absolute top-4 left-4 p-4 dark:text-neutral-100 text-neutral-900 z-10'}>
        <div className={'font-medium'}>This Month:</div>
        <div className={'font-bold text-3xl'}>+300%</div>
      </div>
      <div className={'relative w-full h-full dark:bg-neutral-950 bg-white dark:bg-grid-neutral-700 bg-grid-neutral-200'}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 653 465" className="w-full h-full">
          <motion.path
